            ...
            foreach (var o in lvl)
            {
                ...
                foreach (var p in o.Descendants("Actor"))
                {
                    foreach (var t in p.Descendants("tile"))
                    {
                        ...
                        player.AddComponent(new Player(float.Parse(t.Attribute("x").Value) * 32, float.Parse(t.Attribute("y").Value) * 32, int.Parse(t.Attribute("tx").Value) * 32, int.Parse(t.Attribute("ty").Value) * 32));
                        Scene._AddedGO.Add(player);
                    }
                }
